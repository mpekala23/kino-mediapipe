...
def _get_long_description():
    ...
    return re.sub(
        ...
        open(os.path.join(MP_ROOT_PATH, "README.md"), "rb").read().decode("utf-8"),
    )
...
def _check_bazel():
    ...
    if not shutil.which("bazel"):
        sys.stderr.write(
            "could not find bazel executable. Please install bazel to"
    ...
    try:
        ...
        sys.stderr.write(
            "fail to get bazel version by $ bazel --version: " + str(e.output)
        )
        sys.exit(-1)
...
def _modify_opencv_cmake_rule(link_opencv):
    ...
    if not link_opencv and not IS_WINDOWS:
        content = (
            open(MP_THIRD_PARTY_BUILD, "r")
            .read()
            .replace("OPENCV_SHARED_LIBS = True", "OPENCV_SHARED_LIBS = False")
        )
    ...
    def run(self):
        for target in [
            ...
            _copy_to_build_lib_dir(
                self.build_lib, "mediapipe/tasks/metadata/" + target + "_generated.py"
            )
        for schema_file in [
            ...
            shutil.copyfile(
                schema_file, os.path.join(self.build_lib + "/", schema_file)
            )
