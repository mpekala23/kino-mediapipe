...
CALCULATOR_TO_OPTIONS = {
    "ConstantSidePacketCalculator": constant_side_packet_calculator_pb2.ConstantSidePacketCalculatorOptions,
    "ImageTransformationCalculator": image_transformation_calculator_pb2.ImageTransformationCalculatorOptions,
    "LandmarksSmoothingCalculator": landmarks_smoothing_calculator_pb2.LandmarksSmoothingCalculatorOptions,
    "LogicCalculator": logic_calculator_pb2.LogicCalculatorOptions,
    "ThresholdingCalculator": thresholding_calculator_pb2.ThresholdingCalculatorOptions,
    "TensorsToDetectionsCalculator": tensors_to_detections_calculator_pb2.TensorsToDetectionsCalculatorOptions,
    "Lift2DFrameAnnotationTo3DCalculator": lift_2d_frame_annotation_to_3d_calculator_pb2.Lift2DFrameAnnotationTo3DCalculatorOptions,
}
...
class SolutionBase:
    ...
    def process(
        self,
        input_data: Union[np.ndarray, Mapping[str, Union[np.ndarray, message.Message]]],
    ) -> NamedTuple:
        ...
        for stream_name, data in input_dict.items():
            ...
            if (
                input_stream_type == PacketDataType.PROTO_LIST
                or input_stream_type == PacketDataType.AUDIO
            ):
                ...
                input_stream_type == PacketDataType.IMAGE_FRAME
                or input_stream_type == PacketDataType.IMAGE
            ):
                if data.shape[2] != RGB_CHANNELS:
                    raise ValueError("Input image must contain three channel rgb data.")
                self._graph.add_packet_to_input_stream(
                    stream=stream_name,
                    packet=self._make_packet(input_stream_type, data).at(
                        self._simulated_timestamp
                    ),
                )
            else:
                self._graph.add_packet_to_input_stream(
        ...
        for stream_name in self._output_stream_type_info.keys():
            if stream_name in self._graph_outputs:
                setattr(
                    solution_outputs,
                    stream_name,
                    self._get_packet_content(
                        self._output_stream_type_info[stream_name],
                        self._graph_outputs[stream_name],
                    ),
                )
            else:# Copyright 2020 The MediaPipe Authors.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
...
class SolutionBase:
  ...
  def __init__(
    ...
    validated_graph = validated_graph_config.ValidatedGraphConfig()
    validated_graph.initialize(
        binary_graph_path=os.path.join(root_path, "mediapipe/modules/face_landmark/face_landmark_front_cpu.binarypb"))
  ...
  def __exit__(self, exc_type, exc_val, exc_tb):
    ...
    def _modify_calculator_options(
        self,
        calculator_graph_config: calculator_pb2.CalculatorGraphConfig,
        ...
        nested_calculator_params = generate_nested_calculator_params(calculator_params)
        ...
        for node in calculator_graph_config.node:
            ...
            if node.node_options:
                ...
                # The "options" case for the proto2 syntax as well as the fallback
                # when the calculator doesn't have either "options" or "node_options".
                modify_options_fields(
                    node.options.Extensions[options_type.ext], options_field_list
                )
    ...
    def create_graph_options(
        self, options_message: message.Message, values: Mapping[str, Any]
    ) -> message.Message:
        """Sets protobuf field values.
    ...
    def _set_extension(
        self,
        extension_list: containers.RepeatedCompositeFieldContainer,
        extension_value: message.Message,
    ) -> None:
        """Sets one value in a repeated protobuf.Any extension field."""
        for extension_any in extension_list:
            if extension_any.Is(extension_value.DESCRIPTOR):
    ...
    def _make_packet(
        self, packet_data_type: PacketDataType, data: Any
    ) -> packet.Packet:
        if (
            packet_data_type == PacketDataType.IMAGE_FRAME
            or packet_data_type == PacketDataType.IMAGE
        ):
            return getattr(packet_creator, "create_" + packet_data_type.value)(
                data, image_format=image_frame.ImageFormat.SRGB
            )
    ...
    def _get_packet_content(
        self, packet_data_type: PacketDataType, output_packet: packet.Packet
    ) -> Any:
        """Gets packet content from a packet by type.
